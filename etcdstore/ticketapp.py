...
def deleteticket(ticketid):
    confirm = input(f"Are you sure you want to delete ticket#  {ticketid}? if so, please type 'delete' \n>")
    if confirm == 'delete':
         requests.delete(f"{ETCD}/{ticketid}")
         print(f"{ticketid} has been deleted")
    return

## delete ALL tickets
## use the api parameter ?dir=true&recursive=true to remove a directory
def deletealltickets():
    confirm = input(f"Are you sure you want to delete all tickets? If so, type 'delete' \n>")
    if confirm == 'delete':
        requests.delete(f"{ETCD}?dir=true&recursive=true")
        print("All tickets have been deleted")
    return
